...
// Import the description constants from each tool definition file.
// These must stay in sync with the individual tool files.

// ─── Descriptions imported from tool definition files ────────────────────────
...
const READ_FILE_DESCRIPTION_INTRO =
...
const READ_FILE_MODE_DESCRIPTION =
...
const READ_FILE_EXAMPLES =
	` Example: { path: 'src/app.ts' }` +
	` Example (indentation mode): { path: 'src/app.ts', mode: 'indentation', indentation: { anchor_line: 42 } }`

function getReadFileDefaultDescription(supportsImages: boolean): string {
	const supportsNote = supportsImages ? READ_FILE_SUPPORTS_NOTE_WITH_IMAGES : READ_FILE_SUPPORTS_NOTE_NO_IMAGES
	return (
		READ_FILE_DESCRIPTION_INTRO +
		READ_FILE_MODE_DESCRIPTION +
		READ_FILE_LIMIT_NOTE +
		supportsNote +
		READ_FILE_EXAMPLES
	)
}
...
const USE_MCP_TOOL_DESCRIPTION = `Execute a tool provided by a connected MCP server. Each server may provide different capabilities that you can use to accomplish tasks more effectively.
...
/**
 * Default description for each built-in native tool.
 * These descriptions match exactly what each tool file exports.
 *
 * For `read_file`, the description is dynamic based on image support.
 * Callers should use `getDefaultToolDescription("read_file", supportsImages)`.
 */
export const DEFAULT_TOOL_DESCRIPTIONS: Record<string, string> = {
	execute_command: EXECUTE_COMMAND_DESCRIPTION,
	read_command_output: READ_COMMAND_OUTPUT_DESCRIPTION,
	apply_diff: APPLY_DIFF_DESCRIPTION,
	write_to_file: WRITE_TO_FILE_DESCRIPTION,
	list_files: LIST_FILES_DESCRIPTION,
	search_files: SEARCH_FILES_DESCRIPTION,
	codebase_search: CODEBASE_SEARCH_DESCRIPTION,
	edit: EDIT_DESCRIPTION,
	search_replace: SEARCH_REPLACE_DESCRIPTION,
	edit_file: EDIT_FILE_DESCRIPTION,
	apply_patch: APPLY_PATCH_DESCRIPTION,
	ask_followup_question: ASK_FOLLOWUP_QUESTION_DESCRIPTION,
	attempt_completion: ATTEMPT_COMPLETION_DESCRIPTION,
	switch_mode: SWITCH_MODE_DESCRIPTION,
	new_task: NEW_TASK_DESCRIPTION,
	access_mcp_resource: ACCESS_MCP_RESOURCE_DESCRIPTION,
	use_mcp_tool: USE_MCP_TOOL_DESCRIPTION,
	update_todo_list: UPDATE_TODO_LIST_DESCRIPTION,
	run_slash_command: RUN_SLASH_COMMAND_DESCRIPTION,
	skill: SKILL_DESCRIPTION,
	generate_image: GENERATE_IMAGE_DESCRIPTION,
}

/**
 * Get the default description for a built-in tool.
 * For `read_file`, supportsImages affects the description text.
 */
export function getDefaultToolDescription(toolName: ToolName, supportsImages = false): string | undefined {
	if (toolName === "read_file") {
		return getReadFileDefaultDescription(supportsImages)
	}
	return DEFAULT_TOOL_DESCRIPTIONS[toolName]
}
